...
FROM dias
WHERE fecha = CURRENT_DATE AND salida_hora IS NULL AND entrada_hora IS NOT NULL;
...
DROP USER IF EXISTS 'admin_app'@'localhost';   -- Renombrado para evitar conflicto con ADMIN MySQL

CREATE USER 'empleado'@'localhost'  IDENTIFIED BY 'Empleado#2024!';
CREATE USER 'rrhh'@'localhost'      IDENTIFIED BY 'RRHH#2024!';
CREATE USER 'admin_app'@'localhost' IDENTIFIED BY 'Admin#2024!';

-- ► Permisos EMPLEADO (mínimos necesarios)
GRANT SELECT            ON control_asistencia.empleados    TO 'empleado'@'localhost';
GRANT SELECT, INSERT    ON control_asistencia.dias          TO 'empleado'@'localhost';
GRANT INSERT            ON control_asistencia.auditoria     TO 'empleado'@'localhost';
...
GRANT SELECT, INSERT, UPDATE, DELETE ON control_asistencia.empleados     TO 'rrhh'@'localhost';
GRANT SELECT, INSERT, UPDATE         ON control_asistencia.dias           TO 'rrhh'@'localhost';
GRANT SELECT                         ON control_asistencia.departamentos  TO 'rrhh'@'localhost';
GRANT SELECT                         ON control_asistencia.festivos        TO 'rrhh'@'localhost';
GRANT SELECT                         ON control_asistencia.v_fichajes_completos TO 'rrhh'@'localhost';
GRANT SELECT                         ON control_asistencia.v_resumen_mensual    TO 'rrhh'@'localhost';
GRANT INSERT                         ON control_asistencia.auditoria       TO 'rrhh'@'localhost';
GRANT EXECUTE ON PROCEDURE control_asistencia.registrar_fichaje           TO 'rrhh'@'localhost';
GRANT EXECUTE ON PROCEDURE control_asistencia.obtener_resumen_empleado    TO 'rrhh'@'localhost';
GRANT EXECUTE ON PROCEDURE control_asistencia.registrar_intento_fallido   TO 'rrhh'@'localhost';
GRANT EXECUTE ON PROCEDURE control_asistencia.login_exitoso               TO 'rrhh'@'localhost';

-- ► Permisos ADMIN (control total)
GRANT ALL PRIVILEGES ON control_asistencia.* TO 'admin_app'@'localhost' WITH GRANT OPTION;
