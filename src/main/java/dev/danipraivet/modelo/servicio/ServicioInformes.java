...
public class ServicioInformes {
     ...
     */
    public boolean generarPDF(Empleado empleado, List<Fichaje> fichajes, LocalDate desde, LocalDate hasta, File destino) {
        try {
            ...
            // Cabecera
            Table cabecera = new Table(UnitValue.createPercentArray(new float[]{1, 3})).setWidth(UnitValue.createPercentValue(100));

            // Logo
            URL logoUrl = getClass().getResource("/dev/danipraivet/logo/logo_negro.png");
            if (logoUrl != null) {
                Image logo = new Image(ImageDataFactory.create(logoUrl)).setHeight(50).setAutoScale(false);
                cabecera.addCell(new Cell().add(logo).setBorder(com.itextpdf.layout.borders.Border.NO_BORDER));
            } else {
                cabecera.addCell(new Cell().setBorder(com.itextpdf.layout.borders.Border.NO_BORDER));
            }

            // Título
            Paragraph titulo = new Paragraph("INFORME DE ASISTENCIA").setFontSize(18).setBold().setFontColor(COLOR_CABECERA).setTextAlignment(TextAlignment.RIGHT);
            Paragraph subtitulo = new Paragraph("Período: " + desde.format(FMT_FECHA) + " - " + hasta.format(FMT_FECHA)).setFontSize(10).setFontColor(ColorConstants.GRAY).setTextAlignment(TextAlignment.RIGHT);
            cabecera.addCell(new Cell().add(titulo).add(subtitulo).setBorder(com.itextpdf.layout.borders.Border.NO_BORDER));

            document.add(cabecera);
            document.add(new LineSeparator(new com.itextpdf.kernel.pdf.canvas.draw.SolidLine(1f)).setMarginTop(4).setMarginBottom(8));

            // Datos del empleado
            if (empleado != null) {
                Table infoEmp = new Table(UnitValue.createPercentArray(new float[]{1, 1})).setWidth(UnitValue.createPercentValue(100)).setMarginBottom(12);
                ...
                infoEmp.addCell(celda(empleado.getDepartamento() != null ? empleado.getDepartamento().getNombre() : "-", false));
            ...
            Table tabla = new Table(UnitValue.createPercentArray(anchos)).setWidth(UnitValue.createPercentValue(100));

            // Cabecera de tabla
            String[] columnas = {"Fecha", "Entrada", "Salida", "Horas", "Extras", "Estado"};
            for (String col : columnas) {
                tabla.addHeaderCell(new Cell().add(new Paragraph(col).setBold().setFontColor(ColorConstants.WHITE)).setBackgroundColor(COLOR_CABECERA).setTextAlignment(TextAlignment.CENTER));
            }
            ...
            for (int i = 0; i < fichajes.size(); i++) {
                ...
                tabla.addCell(fila(extras != null && extras.compareTo(BigDecimal.ZERO) > 0 ? extras + "h" : "-", colorFila, true));
            ...
            // Resumen
            Paragraph resumen = new Paragraph(String.format("Total días: %d   |   Total horas trabajadas: %.1f h   |   Horas extra: %.1f h", fichajes.size(), totalHoras, totalExtras)).setFontSize(10).setBold().setMarginTop(10).setTextAlignment(TextAlignment.RIGHT);
     ...
     */
    public boolean generarExcel(Empleado empleado, List<Fichaje> fichajes, LocalDate desde, LocalDate hasta, File destino) {
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            rowPeriodo.createCell(0).setCellValue("Período: " + desde.format(FMT_FECHA) + " - " + hasta.format(FMT_FECHA));
            ...
            if (empleado != null) {
                ...
                rowDep.createCell(1).setCellValue(empleado.getDepartamento() != null ? empleado.getDepartamento().getNombre() : "—");
            }
            ...
            for (int i = 0; i < fichajes.size(); i++) {
                ...
                BigDecimal extras = f.getHorasExtras();
                setValue(row, 4, extras != null && extras.compareTo(BigDecimal.ZERO) > 0 ? extras + "h" : "-", estilo);
            ...
            cResumen.setCellValue(String.format("Total días: %d   |   Horas trabajadas: %.1f h   |   Horas extra: %.1f h", fichajes.size(), totalHoras, totalExtras));
